    ...
    <div class="container-fluid">
        ...
        <section id="who" class="row">
            ...
            <div class="col-md-6 col-sm-6 col-lg-6">
                <p>Basé sur la commune de Châteldon. Cette association a pour but de promouvoir et mettre en place une
                    citoyenneté écologique et de
                    pratiquer la solidarité à l'échelle locale, ainsi que toutes activités et pratiques connexes en lien
                    avec ce qui précède.</p>
                <p>Les membres sont toutes personnes physiques ou morales qui participent à l'association et sont à jour
                    de leurs cotisations. A la création de l'association, les membres sont, de fait, les fondateurs</p>
                <p>L'association est ouverte à toute personne animée par la volonté d'ouvrer à la réalisation de l'objet
                    de l'association, après validation par le Bureau</p>
                <p>Sur notre site internet vous pouvez prendre connaissance des évènements à venir, des évènements passés, de nos ateliers créations et
                    des petits gestes écolos que chacun d'entre nous peut effectuer pour préserver au maximum cette belle planète qui est la notre.</p>
                <p>Un formulaire de contacts vous permet de nous contacter et pour ceux qui le souhaite à souscrire à une newsletter</p>
                <p>Vous pouvez aussi télécharger un bulletin d'adhésion si vous souhaitez rejoindre notre association.</p>
            </div>
            <div class="col-md-6 col-sm-6 col-lg-6">
                <img src="img/cleanteam.jpeg" alt="photo des membres de l'association Châteldon eco-cit' après une récolte de déchets dans la nature en Auvergne" class="img-responsive" width="100%" title="la team Châteldon eco-cit'">
            </div>
        </section>
        <!--Dernier article de la catégorie event-->
        <section class="row" id="events">
        <?php
            $article = 'SELECT * FROM article WHERE category_id=3 ORDER BY date DESC LIMIT 1;';
        ...
        <?php
            $article = 'SELECT * FROM article WHERE category_id=4 ORDER BY date DESC LIMIT 1;';
            $screenArticle = mysqli_query($conn, $article);
        ...
        <!--Dernier article de la catégorie eco_gestes-->
        <section class="row" id="tutos">
        <?php
            $article = 'SELECT * FROM article WHERE category_id=5 ORDER BY date DESC LIMIT 1;';
            $screenArticle = mysqli_query($conn, $article);
